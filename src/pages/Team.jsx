...
export default function Team(){
    ...
    return(
        ...
        <h2 className="text-3xl font-bold m-3 text-accent sm:text-left text-center">Faculty</h2>
        <center>
        ...
        <h2 className="text-3xl font-bold m-3 text-accent sm:text-left text-center">External Panel Expert</h2>
        <ul className="my-5">
        {extern.map((p)=> {
            return (
                <li key={p.id} className="text-cyan_secondary text-xl sm:pl-24">
                    <span className="font-semibold text-white">{p.name}</span>, {p.desig}
                </li>
            )
        })}
        </ul>
        <h2 className="text-3xl font-bold m-3 text-accent sm:text-left text-center">Student</h2>
        <TeamSection />
        </div>
    )
}